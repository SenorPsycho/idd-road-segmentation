...
def build_dataloaders(config: dict):
    ...
    # root_dir in config.yaml is relative to the repo root, not to wherever
    # this script happens to be run from -- resolve it against REPO_ROOT so
    # `python train.py` works the same whether run from Model_Train/ or elsewhere.
...
def run_epoch(
    ...
    If optimizer is given, runs in training mode with backprop.
    Otherwise runs in eval mode under torch.no_grad().

    AMP is used for CUDA forward/loss computation.
    GradScaler is used during training to prevent underflow in FP16 gradients.
    ...
    progress = tqdm(
        ...
        desc="train files" if is_train else "val files",
        unit="file",
        dynamic_ncols=True,
    )
    ...
    try:
        with context:
            for images, masks in loader:
                # non_blocking works together with pin_memory=True and can
                ...
                #
                # On CUDA this allows supported operations to run in FP16/BF16
                # while keeping numerically sensitive operations in FP32.
                ...
                # One dataset item corresponds to one image/file.
...
def train(config_path: str = "config.yaml"):
    ...
    # AMP gradient scaler.
    #
    # It is only needed for CUDA training. On CPU, use None and the training
    # loop simply skips gradient scaling.
    scaler = (
    ...
    for epoch in range(num_epochs):
        encoder_frozen = set_encoder_frozen(
            model,
            epoch,
            freeze_until,
        )

        train_loss, train_ce, train_dice, _ = run_epoch(
            model,
            train_loader,
            criterion,
            device,
        ...
        val_loss, val_ce, val_dice, val_metrics = run_epoch(
            model,
            val_loader,
            criterion,
            device,
        ...
        print(
            f"[epoch {epoch}] frozen={encoder_frozen} "
